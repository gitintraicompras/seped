...
{!! Form::open(array('url'=>'/seped/catimg','method'=>'POST','autocomplete'=>'off', 'enctype'=>'multipart/form-data')) !!}
{{ Form::token() }}
			...
			<div class="row">
				<div class="form-group">
					<div class="col-xs-12 input-group input-group-sm">
				    	<select name="codcat" class="form-control selectpicker" data-live-search="true">
				    		@foreach($categoria as $cat)
				    			<option value="{{$cat->codcat}}">{{$cat->codcat}}-{{$cat->nomcat}}</option>
				    		@endforeach
				    	</select>
					</div>
				</div>
