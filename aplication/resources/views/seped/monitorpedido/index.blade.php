					...
					<td>
                        ...
                        @if (Auth::user()->tipo == 'A' || Auth::user()->tipo == 'S')
	            			...
	            			<a href="" data-target="#modal-delete-{{$t->id}}" data-toggle="modal">
								<button class="btn btn-default btn-pedido fa fa-trash-o" data-toggle="tooltip" title="Eliminar pedido"></button>
							</a>
							@if ($t->estado == 'ANULADO' || $t->estado == 'ENVIADO' )
		  						<!-- MODIFICAR PEDIDO -->
		                        <a href="{{URL::action('AdmonitorpedidoController@edit',$t->id)}}">
		                        	<button class="btn btn-default btn-pedido fa fa-pencil" title="Modificar estatus del pedido">
		                        	</button>
		                        </a>
	                        @endif
						@endif
