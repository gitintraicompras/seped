	          	...
	          	<div @if ($tab=="1") class="tab-pane active" @else class="tab-pane" @endif id="tab_1">
 					<div class="btn-toolbar" role="toolbar" style="margin-bottom: 3px;">
			            <div class="btn-group" role="group" style="width: 100%;">
			                ...
			                &nbsp;&nbsp;
							<a href="{{url('/seped/notiservidor/create')}}" title="Crear notificacioú nueva">
							...
							&nbsp;&nbsp;
						    @include('seped.notiservidor.confirmar', array('metodo'=>'borrar','mensaje'=>'BORRAR TODAS LAS NOTIFICACIONES'))
							...
							<div class="table-responsive">
								<table id="myTable1" class="table table-striped table-bordered table-condensed table-hover">
									...
									<tr>
										<td>{{$t->id}}</td>
										<td>
											<a href="{{URL::action('AdnotiServidorController@show',$t->id)}}"><button style="height: 35px;" class="btn btn-default btn-pedido fa fa-file-o" title="Consultar notificaciún">
											</button>
											</a>
											@if (Auth::user()->tipo == "A" || Auth::user()->tipo == "S")
									...
									@include('seped.notiservidor.delete')
									@endforeach
								</table><br>
							</div>
						</div>
		        ...
		        <div @if ($tab=="2") class="tab-pane active" @else class="tab-pane" @endif id="tab_2">
		        	<div class="btn-toolbar" role="toolbar" style="margin-bottom: 3px;">
			            <div class="btn-group" role="group" style="width: 100%;">
			                ...
			                &nbsp;&nbsp;
			  			    @include('seped.notiservidor.confirmar', array('metodo'=>'leido','mensaje'=>'MARCAR TODAS LAS NOTIFICACIONES COMO LEIDAS'))
						    ...
						    @if (Auth::user()->tipo == "A" || Auth::user()->tipo == "S")
						    &nbsp;&nbsp;
						    @include('seped.notiservidor.confirmar', array('metodo'=>'borrar2','mensaje'=>'BORRAR TODAS LAS NOTIFICACIONES'))
							...
							<div class="table-responsive">
								<table id="myTable2" class="table table-striped table-bordered table-condensed table-hover">
									...
									<tr>
										...
										<td>
											<a href="{{URL::action('AdnotiServidorController@show2',$t->item)}}">
												<button style="height: 35px;" class="btn btn-default btn-pedido fa fa-file-o" title="Consultar notificaciún">
												</button>
											</a>
											@if (Auth::user()->tipo == "A" || Auth::user()->tipo == "S")
											<a href="" data-target="#modal-delete2-{{$t->item}}" data-toggle="modal">
												<button style="height: 35px;" class="btn btn-default btn-pedido fa fa-trash-o" data-toggle="tooltip" title="Eliminar notificaciún">
												</button>
											</a>
											@endif
									...
									@include('seped.notiservidor.delete2')
									@endforeach
								</table><br>
							</div>
						</div>
...
function tdclick(e) {
    ...
    $.ajax({
	  ...
	  success:function(data) {
	    ...
	    var url = "{{url('/seped/notiservidor?tab=2')}}";
